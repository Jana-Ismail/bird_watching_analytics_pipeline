...
RAW_HISTORIC_PREFIX = f"weather/fire/source=nasa-firms/region={CA_REGION_CODE}/frequency=historic"

DOWNLOAD_DATE = get_current_utc_timestamp("%Y-%m-%d")


def convert_csv_to_parquet_arrow(raw_bytes, separator=",", metadata=None):
    """
...
def get_csv_files_from_minio(bucket_name=MINIO_RAW_BUCKET_NAME):
    """
    List only .csv files from MinIO under the raw historic txt prefix.
    """
    csv_prefix = f"{RAW_HISTORIC_PREFIX}/file_format=csv/"
    minio_client = connect_to_minio()
    objects = minio_client.list_objects(bucket_name, prefix=csv_prefix, recursive=True)
...
def process_raw_file(object_name, timestamp, bucket_name=MINIO_RAW_BUCKET_NAME, separator=","):
    """
    Download a raw historic CSV file, convert to Parquet, and upload back
    ...
    # Metadata
    metadata = {
        "_source": "manual historic NASA FIRMS .csv upload",
    ...
    parquet_buffer = convert_csv_to_parquet_arrow(raw_bytes, separator=separator, metadata=metadata)
...
def main():
    ...
    files = get_csv_files_from_minio()
    if not files:
        logger.warning("No historic CSV files found to process")
        return
